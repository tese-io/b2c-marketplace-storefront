...
const PROOF = [
  ...
  {
    value: "8",
    labelKey: "sustainableCategories",
    icon: (
      <svg className="h-5 w-5 text-tese-lime" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75} aria-hidden>
    ...
    <section className="relative w-full overflow-hidden bg-white">
      ...
      <div className="tese-container relative py-14 lg:py-20 text-center">
        <p className="text-sm text-secondary font-medium">
          {sector.id === "all"
            ? t("eyebrow")
            : t("personalisedFor", { sector: sector.label.toLowerCase() })}
        </p>
          ...
          {sector.id === "all" ? (
            <>
              ...
              <span className="block mt-2 lg:mt-3">
                {t("headlineFor")}{" "}
                <span className="text-tese-lime">{t("headlineAccent")}</span>
              </span>
            </>
          ) : (
            <>
          ...
          {PROOF.map((s) => (
            <div key={s.labelKey} className="flex items-center gap-2.5">
              ...
              <div className="text-left">
                ...
                <p className="text-[12px] text-secondary">{t(`proof.${s.labelKey}`)}</p>
              </div>
            </div>
          ))}
        </div>
